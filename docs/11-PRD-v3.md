...
**완료 조건 (체크 가능한 형태)**
- [ ] 시터 계정 로그인 후 새로고침·기기 교체에도 상태 유지
- [ ] 초대 링크로 연 보호자가 **계정 없이** 아이 등록 → 일정 전달까지 완료
- [ ] 시터가 사진 인증 제출 → **다른 기기의 보호자 화면**에 30초 내 반영
- [ ] 사진이 Storage에 저장되고 새로고침 후에도 보임 (localStorage 미사용)
- [ ] 재제출 시 판정 무효화·이의 보존이 **서버에서** 강제됨 (클라이언트 우회 불가)
- [ ] 보호자가 동의 토글을 켠 계약만 공개 링크로 열림, 끈 계약은 404
